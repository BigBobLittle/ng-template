...
import { CoreService } from '../_core/service/core/core.service';
...
//  }
   ...
   ngOnInit() {
      ...
      this._mediaSubscription = this.media.media$.subscribe((change: MediaChange) => {
         ...
         if(this.isMobile || ((this.router.url === '/ecommerce/shop' || this.router.url === '/ecommerce/checkout' || this.router.url === '/ecommerce/invoice' || this.router.url === '/courses/courses' || this.router.url === '/courses/courses-list' || this.router.url === '/courses/course-detail') && window.innerWidth<1920)) {
            this.coreService.sidenavMode = 'over';  
            this.coreService.sidenavOpen = false;
         }
         else {
